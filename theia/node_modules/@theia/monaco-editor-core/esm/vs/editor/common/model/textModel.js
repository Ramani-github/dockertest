...
var BracketSearchCanceled = /** @class */ (function () {
    function BracketSearchCanceled() {
        this._searchCanceledBrand = undefined;
    }
    BracketSearchCanceled.INSTANCE = new BracketSearchCanceled();
    return BracketSearchCanceled;
}());
...
var TextModel = /** @class */ (function (_super) {
    ...
    //#endregion
    function TextModel(source, creationOptions, languageIdentifier, associatedResource, undoRedoService) {
        ...
        var _a = createTextBuffer(source, creationOptions.defaultEOL), textBuffer = _a.textBuffer, disposable = _a.disposable;
        ...
        _this._commandManager = new EditStack(_this, undoRedoService);
    ...
    TextModel.prototype.getTextBuffer = function () {
        this._assertNotDisposed();
        return this._buffer;
    };
    ...
    TextModel.prototype.setValue = function (value) {
        ...
        var _a = createTextBuffer(value, this._options.defaultEOL), textBuffer = _a.textBuffer, disposable = _a.disposable;
    ...
    TextModel.prototype._setValueFromTextBuffer = function (textBuffer, textBufferDisposable) {
        this._assertNotDisposed();
    ...
    TextModel.prototype.mightContainUnusualLineTerminators = function () {
        return this._buffer.mightContainUnusualLineTerminators();
    };
    TextModel.prototype.removeUnusualLineTerminators = function (selections) {
        if (selections === void 0) { selections = null; }
        var matches = this.findMatches(strings.UNUSUAL_LINE_TERMINATORS.source, false, true, false, null, false, 1073741824 /* MAX_SAFE_SMALL_INTEGER */);
        this._buffer.resetMightContainUnusualLineTerminators();
        this.pushEditOperations(selections, matches.map(function (m) { return ({ range: m.range, text: null }); }), function () { return null; });
    };
    ...
    TextModel.prototype.getInitialUndoRedoSnapshot = function () {
        this._assertNotDisposed();
    ...
    TextModel.prototype._overwriteVersionId = function (versionId) {
        this._versionId = versionId;
    };
    TextModel.prototype._overwriteAlternativeVersionId = function (newAlternativeVersionId) {
        this._alternativeVersionId = newAlternativeVersionId;
    };
    TextModel.prototype._overwriteInitialUndoRedoSnapshot = function (newInitialUndoRedoSnapshot) {
    ...
    TextModel.prototype.getEndOfLineSequence = function () {
        this._assertNotDisposed();
        return (this._buffer.getEOL() === '\n'
            ? 0 /* LF */
            : 1 /* CRLF */);
    ...
    TextModel.prototype.findMatches = function (searchString, rawSearchScope, isRegex, matchCase, wordSeparators, captureMatches, limitResultCount) {
        ...
        if (rawSearchScope !== null) {
            ...
            if (rawSearchScope.every(function (searchScope) { return Range.isIRange(searchScope); })) {
                searchRanges = rawSearchScope.map(function (searchScope) { return _this.validateRange(searchScope); });
            }
        }
        if (searchRanges === null) {
            searchRanges = [this.getFullModelRange()];
        }
        searchRanges = searchRanges.sort(function (d1, d2) { return d1.startLineNumber - d2.startLineNumber || d1.startColumn - d2.startColumn; });
        ...
        if (!isRegex && searchString.indexOf('\n') < 0) {
            ...
            var searchData_1 = searchParams.parseSearchRequest();
            if (!searchData_1) {
                return [];
            }
            matchMapper = function (searchRange) { return _this.findMatchesLineByLine(searchRange, searchData_1, captureMatches, limitResultCount); };
        }
        else {
            matchMapper = function (searchRange) { return TextModelSearch.findMatches(_this, new SearchParams(searchString, isRegex, matchCase, wordSeparators), searchRange, captureMatches, limitResultCount); };
        }
        return uniqueSearchRanges.map(matchMapper).reduce(function (arr, matches) { return arr.concat(matches); }, []);
    };
    ...
    TextModel.prototype.popStackElement = function () {
    ...
    TextModel.prototype._validateEditOperation = function (rawOperation) {
        if (rawOperation instanceof model.ValidAnnotatedEditOperation) {
            return rawOperation;
        }
        return new model.ValidAnnotatedEditOperation(rawOperation.identifier || null, this.validateRange(rawOperation.range), rawOperation.text, rawOperation.forceMoveMarkers || false, rawOperation.isAutoWhitespaceEdit || false, rawOperation._isTracked || false);
    };
    TextModel.prototype._validateEditOperations = function (rawOperations) {
        var result = [];
    ...
    TextModel.prototype._applyUndo = function (changes, eol, resultingAlternativeVersionId, resultingSelection) {
        var _this = this;
        var edits = changes.map(function (change) {
            var rangeStart = _this.getPositionAt(change.newPosition);
            var rangeEnd = _this.getPositionAt(change.newEnd);
            return {
    ...
    TextModel.prototype._applyRedo = function (changes, eol, resultingAlternativeVersionId, resultingSelection) {
        var _this = this;
        var edits = changes.map(function (change) {
            var rangeStart = _this.getPositionAt(change.oldPosition);
            var rangeEnd = _this.getPositionAt(change.oldEnd);
            return {
                range: new Range(rangeStart.lineNumber, rangeStart.column, rangeEnd.lineNumber, rangeEnd.column),
                text: change.newText
            };
    ...
    TextModel.prototype._applyUndoRedoEdits = function (edits, eol, isUndoing, isRedoing, resultingAlternativeVersionId, resultingSelection) {
        try {
            this._onDidChangeDecorations.beginDeferredEmit();
            this._eventEmitter.beginDeferredEmit();
    ...
    TextModel.prototype.applyEdits = function (rawOperations, computeUndoEdits) {
        if (computeUndoEdits === void 0) { computeUndoEdits = false; }
        try {
            this._onDidChangeDecorations.beginDeferredEmit();
            this._eventEmitter.beginDeferredEmit();
    ...
    TextModel.prototype._doApplyEdits = function (rawOperations, computeUndoEdits) {
        ...
        if (contentChanges.length !== 0) {
            ...
            for (var i = 0, len = contentChanges.length; i < len; i++) {
                ...
                var _a = __read(countEOL(change.text), 3), eolCount = _a[0], firstLineLength = _a[1], lastLineLength = _a[2];
    ...
    TextModel.prototype.setSemanticTokens = function (tokens, isComplete) {
    ...
    TextModel.prototype.hasCompleteSemanticTokens = function () {
        return this._tokens2.isComplete();
    };
    TextModel.prototype.hasSomeSemanticTokens = function () {
        return !this._tokens2.isEmpty();
    };
    TextModel.prototype.setPartialSemanticTokens = function (range, tokens) {
    ...
    TextModel.prototype.getWordAtPosition = function (_position) {
        ...
        var _a = __read(TextModel._findLanguageBoundaries(lineTokens, tokenIndex), 2), rbStartOffset = _a[0], rbEndOffset = _a[1];
        ...
        if (tokenIndex > 0 && rbStartOffset === position.column - 1) {
            ...
            var _b = __read(TextModel._findLanguageBoundaries(lineTokens, tokenIndex - 1), 2), lbStartOffset = _b[0], lbEndOffset = _b[1];
        ...
        if (tokenIndex > 0 && lineTokens.getStartOffset(tokenIndex) === position.column - 1) {
            ...
            if (prevModeBrackets && !ignoreBracketsInToken(lineTokens.getStandardTokenType(prevTokenIndex))) {
                ...
                if (foundBracket && foundBracket.startColumn <= position.column && position.column <= foundBracket.endColumn) {
                    var foundBracketText = lineText.substring(foundBracket.startColumn - 1, foundBracket.endColumn - 1).toLowerCase();
                    var r = this._matchFoundBracket(foundBracket, prevModeBrackets.textIsBracket[foundBracketText], prevModeBrackets.textIsOpenBracket[foundBracketText], null);
                    if (r) {
                        if (r instanceof BracketSearchCanceled) {
                            return null;
                        }
    ...
    TextModel.prototype._matchFoundBracket = function (foundBracket, data, isOpen, continueSearchPredicate) {
        if (!data) {
            return null;
        }
        var matched = (isOpen
    ...
    TextModel.prototype._findMatchingBracketUp = function (bracket, position, continueSearchPredicate) {
    ...
    TextModel.prototype._findMatchingBracketDown = function (bracket, position, continueSearchPredicate) {
    ...
    TextModel.prototype.findEnclosingBrackets = function (_position, maxDuration) {
        ...
        if (typeof maxDuration === 'undefined') {
            ...
            continueSearchPredicate = function () {
                return (Date.now() - startTime_1 <= maxDuration);
            };
        }
        var position = this.validatePosition(_position);
        var lineCount = this.getLineCount();
            ...
            while (true) {
                ...
                if (bracket) {
                    ...
                    if (counts[bracket.index] === -1) {
                        return _this._matchFoundBracket(r, bracket, false, continueSearchPredicate);
                    }
                }
                searchStartOffset = r.endColumn - 1;
            }
...
var DidChangeDecorationsEmitter = /** @class */ (function (_super) {
    ...
    function DidChangeDecorationsEmitter() {
        ...
        _this._affectsMinimap = false;
        _this._affectsOverviewRuler = false;
        return _this;
    }
    ...
    DidChangeDecorationsEmitter.prototype.endDeferredEmit = function () {
        ...
        if (this._deferredCnt === 0) {
            if (this._shouldFire) {
                ...
                this._actual.fire(event_1);
            }
        }
    };
    DidChangeDecorationsEmitter.prototype.checkAffectedAndFire = function (options) {
...
var DidChangeContentEmitter = /** @class */ (function (_super) {
    ...
    DidChangeContentEmitter.prototype.endDeferredEmit = function (resultingSelection) {
        if (resultingSelection === void 0) { resultingSelection = null; }
        this._deferredCnt--;
        if (this._deferredCnt === 0) {
